...
from dataset import load_simple_data, simple_collate_fn, load_esposalles_data_probabilistic
...
# os.environ["CUDA_VISIBLE_DEVICES"] = "5"  # Commented out - set via environment or command line
...
def simple_train_probabilistic(category_percentages=None, epochs=EPOCHS, wandb_project="trocr-probabilistic", run_name=None):
    """
    NEW: Probabilistic training function where recognition is always included 
    ...
    # Initialize WandB
    if run_name is None:
        run_name = f"prob_run_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    logger.info("Loading probabilistic datasets...")
    data_train_prob, data_test_prob, data_train_rec, data_test_rec = load_esposalles_data_probabilistic(
        category_percentages=category_percentages
    )
...
if __name__ == '__main__':
    category_percentages = {
        ...
        "POSITIONAL_ANALYSIS": 25,
        "STRUCTURAL_ANALYSIS": 30,
        "BOUNDARY_ANALYSIS": 15
    }
    best_wer, best_epoch = simple_train_probabilistic(
        category_percentages=category_percentages,
        epochs=50,
        wandb_project="BERT-2Projection-Esposalles-Probabilistic",
        run_name="30-25-30-15"
    )
