...
from dataset import load_datasets, collate_fn
from model import ImageTextTrOCR
import logging
import wandb
from datetime import datetime

# Environment setup
# os.environ["CUDA_VISIBLE_DEVICES"] = "3"  # Commented out - set via environment or command line
...
def train_process(augmentation=False, epochs=EPOCHS, wandb_project="trocr-training", 
                 wandb_name=None, run_name=None):
    """
    Main training function adapted for simplified dataset
    ...
    for epoch in tqdm(range(1, epochs + 1)):
        ...
        if epoch % 2 == 0:
            ...
            with torch.no_grad():
                for data in train_loader:
                    ...
                    # Use autoregressive generation (no teacher forcing)
            ...
            # Calculate WER and CER for training set
            ...
            # Calculate WER and CER for test set
            ...
            # Log prediction examples to WandB every 10 epochs (since eval is every 2 epochs)
            if epoch % 10 == 0:
            ...
            # For non-evaluation epochs, set dummy values to maintain early stopping logic
            ...
            # Add placeholder metrics for non-eval epochs
            epoch_metrics.update({
                "validation_metric": validation_metric,
                "early_stop_counter": early_stop_counter
            })
        ...
        # Log all epoch metrics to WandB once per epoch
        wandb.log(epoch_metrics, step=epoch)
...
if __name__ == '__main__':
    ...
    best_res_normal, run_name_normal = train_process(
        ...
        epochs=50,
        wandb_project="trocr",
        wandb_name="TrOCR-AdamW-5e6-BS6",
        run_name="TrOCR-AdamW-5e6-BS6"
    )